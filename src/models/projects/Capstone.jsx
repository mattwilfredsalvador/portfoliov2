...
const Capstone = ( {isVisible, setIsVisible, ...props}) => {
  ...
  useFrame((_, delta) => {
    if (isHovered){
      tabletRef.current.position.lerp(new THREE.Vector3(-10, -63, -37), 0.1)
    } else if(!isHovered){
      tabletRef.current.rotation.x += 0.1 * delta
      tabletRef.current.rotation.y += 0.1 * delta
      tabletRef.current.position.lerp(new THREE.Vector3(-10, -65, -40), 0.1)
    }
