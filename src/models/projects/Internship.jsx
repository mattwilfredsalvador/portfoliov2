...
import textureImage from "../../assets/images/attendance.gif"

const Internship = ( {isVisible, setIsVisible, ...props}) => {
  ...
  useFrame((_, delta) => {
    if (isHovered){
      tabletRef.current.position.lerp(new THREE.Vector3(-30, -78, -37), 0.1)
    } else if(!isHovered){
      tabletRef.current.rotation.x += 0.01 * delta
      tabletRef.current.rotation.y += 0.1 * delta
      tabletRef.current.position.lerp(new THREE.Vector3(-30, -80, -40), 0.1)
    }
