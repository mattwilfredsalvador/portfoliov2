...
const Kriya = ( {isVisible, setIsVisible, ...props}) => {
  ...
  useFrame((_, delta) => {
    if (isHovered){
      tabletRef.current.position.lerp(new THREE.Vector3(-30, -63, -37), 0.1)
    } else if(!isHovered){
      tabletRef.current.rotation.y += 0.10 * delta
      tabletRef.current.position.lerp(new THREE.Vector3(-30, -65, -40), 0.1)
    }
